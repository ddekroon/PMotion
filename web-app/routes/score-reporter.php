<?php
	...
	$app->get('/score-reporter[/{sportID}[/{leagueID}[/{teamID}]]]', function(Request $request, Response $response) {
		...
		if($sport->getId() == null) {
			throw new Exception('Invalid sport ID when opening score reporter', 400);
		}
		...
		if($teamID > 0) {
			...
			if(isset($team) && $team->getId() != null) {
				...
				if($team->getLeague() !== null) {
					...
					if($team->getLeague() != null) {
						$changed = $leaguesController->setLeagueWeekInScoreReporter($team->getLeague());
						
						//Reload the reference to the team so that the further reference to league has the correct date.
						if($changed) {
							$team = Models_Team::withID($this->db, $this->logger, $teamID);
						}
					}
					...
					return $this->view->render($response, "score-reporter/score-reporter-matches.phtml", [
							...
							"router" => $this->router,
							"isDashboard" => false,
							"leagueDate" => $leaguesController->getActiveDate($team->getLeague())
						]
					);	
				}
			}
