...
abstract class Includes_Helper {
	
	public static function removeCommonWords($input){
		
		$keywords = explode(' ', $input); //break all search terms by space
		...
		foreach($keywords as $keyword) {
			...
			$curKeyword = preg_replace('/^('.implode('|',$commonWords).')$/','',$keyword);
						
			if(strlen($curKeyword) > 2) {
				$toReturn[] = $curKeyword;
			}
		}
