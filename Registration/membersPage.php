...
$seasonsArray = $dbConnection->query($seasonsQuery) or die ('Date error'.mysqli_error());
while($seasonNode = mysqli_fetch_array($seasonsArray)) {
	...
	if(secondIsLater($openDate, $currentDate) == 1 AND secondIsLater($currentDate, $closeDate) ==1){ //within registration time
		$dbConnection->query("UPDATE $seasonsTable SET season_available_registration = 1 WHERE season_id = $seasonID");
		$dbConnection->query("UPDATE $leaguesTable SET league_available_for_registration = 1 WHERE league_season_id = $seasonID");
	} else { //not as much
		$dbConnection->query("UPDATE $seasonsTable SET season_available_registration = 0 WHERE season_id = $seasonID");
		$dbConnection->query("UPDATE $leaguesTable SET league_available_for_registration = 0 WHERE league_season_id = $seasonID");
	}
}
	
//Get current season data
$seasonQuery=$dbConnection->query("SELECT * FROM $seasonsTable WHERE season_available_registration = 1");
$seasonArray=mysqli_fetch_array($seasonQuery);
...
if($seasonObj['regBySport'] == 1) {
	$sportsQuery = $dbConnection->query("SELECT * FROM $sportsTable 
		...
		or die('ERROR getting sport data - '.mysqli_error());
	$numSports = 1;
	$sportsObj = array();
	$lastSportID = 0;
	while($sportArray = mysqli_fetch_array($sportsQuery)) {
...
$pastSeasonQuery = $dbConnection->query("SELECT * FROM $seasonsTable WHERE season_id = $lastSeasonID");
$pastSeasonArray = mysqli_fetch_array($pastSeasonQuery);
...
if(isset($_POST['delete'])) {
	if(isset($_POST['team'])) {
		foreach($_POST['team'] as $teamID) {
			$dbConnection->query("UPDATE $teamsTable SET team_deleted = 1 WHERE team_id = $teamID");
			//If the team being deleted is in the season users are currently able to register for, set its number in league to 0.
			$seasonQuery=$dbConnection->query("SELECT * FROM $seasonsTable 
				...
				WHERE team_id = $teamID") or die('ERROR getting season/league data for team '.$teamID.' - '.mysqli_error());
			$seasonArray=mysqli_fetch_array($seasonQuery);
			...
			if ($seasonAvailableForReg == 1 && $teamNumInLeague > 0) {
				...
				$dbConnection->query("UPDATE $teamsTable SET team_num_in_league = 0 WHERE team_id = $teamID");
...
$teamsQuery=$dbConnection->query($teamsQueryString) or die('ERROR getting teams data '.$userID.' - '.mysqli_error());
$count=0;
if(mysqli_num_rows($teamsQuery) > 0) {
	while ($teamsArray=mysqli_fetch_array($teamsQuery)){
...
<html>
    ...
    <body>		
        ...
        <div class='container'>
			...
			<div class='pastTeams'>
			<?php if(mysqli_num_rows($teamsQuery) > 0){ ?>
			<table>
				<tr>
					<th colspan=7>
...
function fixTeamNumbers($leagueID, $teamID) {
	global $teamsTable, $dbConnection;
	$teamNum = 0;
	$deleteNum = 0;
	$teamsQuery = $dbConnection->query("SELECT * FROM $teamsTable WHERE team_league_id = $leagueID AND team_num_in_league > 0 ORDER BY team_num_in_league ASC")
		or die('ERROR getting teams to change #s'.mysqli_error());
	while($teamArray = mysqli_fetch_array($teamsQuery)) {
	...
	if($deleteNum != 0) {
		for($i=0; $i< $teamNum; $i++) {
			if($i > $deleteNum) {
				$dbConnection->query("UPDATE $teamsTable SET team_num_in_league = team_num_in_league-1 WHERE team_id = $teamID[$i]") 
					or die('ERROR updating team numbers '.mysqli_error());
			}
		}
	} else {
