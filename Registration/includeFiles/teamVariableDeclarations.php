...
function getSubmittedValues($teamID) {
	global $player, $leaguesTable, $sportsTable;
	$team = new Team();
...
function getLeagueName($tempLeagueName, $teamsFull, $leagueDay, $regFee) {
	...
	$teamsFull > 0 ? $leagueName .= '- Full - Waiting List ' : $leagueName .= '';
	$leagueName .= "- $leagueDay (Team Fee: $".number_format($regFee, 2).')';
