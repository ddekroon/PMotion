...
function body($userID){
	...
	$body.='<tr><th colspan=4>How they heard about us: '.$aboutUsMethod.' - '.$aboutUsText.'</th></tr>';
	$body.='<tr><td colspan=4 align=center>---------------------------------------------------------------</td></tr>';
...
function mailForm($userID, $comSub, $saveFilter = 0){
	...
	$title.='<tr><th colspan=3 align=center>-- Registration Confirmation --</th></tr>';
	  ...
	  $secondary='<tr><td colspan=3 align=center>'.$returningString.'Team Registered for <B>'.stripslashes($teamObj->teamSportName).' - '.stripslashes($teamObj->teamLeagueName).' - '.stripslashes($teamSeasonName).'</B></td></tr>';
	$body=body($userID);
	...
	$altMessage.='</table>';
	if($saveFilter == 0) {
		sendEmailsBcc(array($player[0]->playerEmail,),'info@perpetualmotion.org', $subject,  $message);
	}
