...
export function submitScoreSubmission () {
  return (dispatch, getState) =>
    new Promise(async (resolve, reject) => {
      // Validation checks
      if (
        !isScoreSubmissionValid(getState().scoreSubmission, getState().leagues)
      ) {
      ...
      console.log(
        'Submit score submission: ' + JSON.stringify(getState().scoreSubmission)
      )

      return fetch(
        'http://data.perpetualmotion.org/web-app/api/score-submission',
        {
          method: 'POST',
          headers: {
        ...
        .catch(err => {
          ...
          ToastHelpers.showToast(
            Enums.messageTypes.Error,
            ErrorMessages.errorSendingToServer
          )
...
function isScoreSubmissionValid (scoreSubmission, leagues) {
  if (
    !ValidationHelpers.isValidId(scoreSubmission.sportId) ||
  ...
  for (var i = 0; i < parseInt(league.numMatches, 10); i++) {
    ...
    if (
      !ValidationHelpers.isValidId(curMatch.oppTeamId) ||
    ...
    for (
      var gameNum = 0;
      gameNum < parseInt(league.numGamesPerMatch, 10);
      gameNum++
    ) {
