...
* August 12, 2013
* 
* Included by Login/index.php to actually do the logging in. Already has config included so it has all the data required.
****************************/
...
$checkUsername = $dbConnection->query("SELECT * FROM $userTable WHERE user_username = '$postUsername'")or die($dbConnection->error);
...
$nameInfo = $checkUsername->fetch_array();
...
if ($formRight == true && $userRight == true && $passRight == true){
	...
	VALUES ($userID, '$postUsername', '$historyType', NOW())";
	
	$add_history = $dbConnection->query($insertHistory) or die($dbConnection->error);
