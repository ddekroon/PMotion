...
function getActiveTeams($leagueID) {
	...
	global $totalWins, $totalLosses, $totalTies, $teamMaxWeek;
	...
	$queryString = "SELECT
		...
		FROM $spiritScoresTable INNER JOIN $scoreSubmissionsTable 
		...
		WHERE league_id = $leagueID AND spirit_score_ignored = 0 AND spirit_score_edited_value > 0 
		...
		AND team_league_id = $leagueID AND team_num_in_league > 0 AND team_dropped_out = 0
		GROUP BY team_id";
	...
	while($teamObj = $teamsQuery->fetch_object()){ 
		...
		$totalWins += $teamObj->team_wins;
		$totalLosses += $teamObj->team_losses;
		$totalTies += $teamObj->team_ties;
