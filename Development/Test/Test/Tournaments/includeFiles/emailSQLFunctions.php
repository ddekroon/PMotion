...
function getPlayerData($sportID, $leagueID, $dayNumber, $seasonID, $orderBy, $direction) {
	...
	$playersQuery = "SELECT * FROM $playersTable INNER JOIN $teamsTable ON $teamsTable.team_id = $playersTable.player_team_id 
...
function getCaptainData($sportID, $leagueID, $dayNumber, $seasonID, $orderBy, $direction) {
	...
	if($filterChosen == 1) {
		...
		$playersQuery = "SELECT * FROM $playersTable 
...
function getPastTournamentData($tourneyID, $year, $orderBy, $direction) {
	...
	$playersQuery = "SELECT * FROM $tournamentPlayersTable INNER JOIN $tournamentsTable ON
	...
	while($player = $result->fetch_object()) {
		if(!in_array(strtolower($player->tournament_player_email), $curPlayerEmails) 
			...
			if($teamID != 0) {
				$teamQuery = "SELECT * FROM $tournamentTeamsTable WHERE tournament_team_id = $teamID LIMIT 1";
