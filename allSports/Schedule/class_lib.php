...
class Schedule{
	...
	public function setTeamListTable()
	{
		...
		while($key = mysql_fetch_assoc($result))
		{
			...
			{
				$team_array[] = array($key['team_num_in_league'],$key['team_name'],$key['team_id'],$key['team_dropped_out']);
			}
		}
	...
	function makeHeaders($ttl, $tmlst){
		...
		$headers .= "<TABLE align='center' class='schedule' style='width:95%;max-width:825px;min-width:100px;'><TR><td id='title'>$ttl</td></TR>";
	...
	function storeScheduleHTML(){
		...
		for($week_number = 1; $week_number <= $this->getNumberOfWeeksInLeague(); $week_number++){
			...
			if ($numGamesPerRow == 2) {			
				...
				$this->schedule_html[$week_number] .= "<TR><td id='weekCell'><table align='center' class='schedWeek' style='width:95%;max-width:825px;'>";
			} //I think this will give correct dimensions, but not sure - unable to test at the moment
