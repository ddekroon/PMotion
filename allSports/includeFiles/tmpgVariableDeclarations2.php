...
function getMatchData($teamObjs, $teamID) 
{
	...
	if(!($datesQuery = $dbConnection->query("SELECT scheduled_match_team_id_1, scheduled_match_team_id_2, scheduled_match_time, date_id, date_description,   venue_short_show_name, venue_link FROM $scheduledMatchesTable 
...
function getTeamsData($teamID, $newFlag) 
{
	...
	{
		$queryString = "SELECT
			team_id, team_league_id, team_name, team_wins, team_losses, team_ties, team_most_recent_week_submitted, team_pic_name, team_dropped_out,
			...
			FROM (SELECT * FROM $teamsTable WHERE team_league_id = (SELECT team_league_id FROM $teamsTable WHERE team_id = $teamID)
				AND ((team_num_in_league) OR team_id = 1)) as teamTable
	...
	else 
	{
		$queryString = "SELECT
			...
			LEFT JOIN $leaguesTable ON $leaguesTable.league_id = teamTable.team_league_id
			LEFT JOIN $scoreSubmissionsTable ON ($scoreSubmissionsTable.score_submission_opp_team_id = teamTable.team_id AND (score_submission_ignored = 0
			OR score_submission_is_phantom = 1))
