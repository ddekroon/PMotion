...
{ 
	global $sportID; ?>
		...
		<div class="Column" id='opponent'>
        	...
        	<?php 
			if ($matchObj->oppTeamID != 1) 
			{
				if ($matchObj->oppTeamName[0] != '*')
				{
					print "<a href='teamPage.php?teamID=".$matchObj->oppTeamID."'>".$matchObj->oppTeamName.'</a>';
				}
				else
				{
					// Gets rid of the '*' that designates if a team dropped or not
					for($i=1;$i<strlen($matchObj->oppTeamName);$i++)
					{
						print $matchObj->oppTeamName[$i];
					}
				}				
			}
			else
