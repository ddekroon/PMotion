function checkFields(matchnum, gamenum) {
	...
	var form = document.getElementById('trackReporterID');
...
function reloadPage() {
	...
	var leagueID=form.elements['leagueID'].options[form.elements['leagueID'].options.selectedIndex].value;
	self.location='scoreTracker.php?sportID=' + sportID + '&leagueID=' + leagueID;
}
...
function secondReload() {
	...
	var leagueID= form.elements['leagueID'].options[form.elements['leagueID'].options.selectedIndex].value;
	var teamID= form.elements['teamID'].options[form.elements['teamID'].options.selectedIndex].value;
	self.location='scoreTracker.php?sportID=' + sportID + '&leagueID=' + leagueID +'&teamID=' + teamID;
}
...
{
	var form = document.getElementById('trackReporterID');
	var score = document.getElementsByName("usScore");
	score = score - 1;
	document.getElementsByName("usScore") = score;
}

function addOneUs()
{
	var form = document.getElementById('trackReporterID');
	var score = form.elements['usScore'].value;
	score = score + 1;
	document.trackReporter.usScore.value = score;
}
