...
function getTeamsData() {
	...
	$teamsQuery = mysql_query("SELECT team_id FROM $teamsTable INNER JOIN $playersTable ON $playersTable.player_team_id = $teamsTable.team_id
	...
	$teamsQuery = mysql_query("SELECT team_id, team_name, league_name, team_league_id, league_day_number, league_sport_id FROM $teamsTable INNER JOIN $leaguesTable ON $leaguesTable.league_id = $teamsTable.team_league_id
		INNER JOIN $seasonsTable ON $seasonsTable.season_id = $leaguesTable.league_season_id 
...
function printCaptainNode($curTeam, $iteratorNum, $numEditedTeams, $playerObj) { ?>
    ...
    <tr>
    	<td style="width:20px;">
			...
			<?php print '<a href="editTeam.php?sportID='.$curTeam->teamSportID.'&leagueID='.$curTeam->teamLeagueID.'&teamID='.$curTeam->teamID.'">'.$curTeam->teamName.'</a>'; ?>
