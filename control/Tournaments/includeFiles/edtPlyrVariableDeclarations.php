...
function getDBPlayerData($playerID) {
	...
	$player = new Player();
	if($playerID != 0) {
		$playerQuery = mysql_query("SELECT * FROM $tournamentPlayersTable WHERE tournament_player_id = $playerID") or die ('ERROR getting player DB data '.mysql_error());
		$playerArray = mysql_fetch_array($playerQuery);
		$player->playerFirstName = $playerArray['tournament_player_firstname'];
		$player->playerLastName = $playerArray['tournament_player_lastname'];
		$player->playerEmail = $playerArray['tournament_player_email'];
		$player->playerGender = $playerArray['tournament_player_gender'];
		$player->playerNote = $playerArray['tournament_player_note']; 
	}
	return $player;
}
...
function getPostPlayerData() {
	...
	$player->playerGender = $_POST['gender'];
...
function getPlayerDD($tourneyID, $isLeagues, $leagueID, $isTeams, $teamID, $isCards, $playerID) {
	global $tournamentPlayersTable;
	$playersDropDown = '';
	
	if($isLeagues == 1) {
		$leagueFilter = "AND tournament_player_league_id = $leagueID";
		$teamNumFilter = 'AND tournament_player_num_in_league > 0';
		$teamFilter = '';
		$orderFilter = 'tournament_player_num_in_league ASC';
	} else {
		$leagueFilter = '';
		$teamNumFilter = 'AND tournament_player_num_in_tournament > 0';
		$teamFilter = '';
	...
	if ($isTeams == 1) {
		$leagueFilter = '';
		$teamNumFilter = '';
		$teamFilter = "AND tournament_player_team_id = $teamID";
		$orderFilter = 'tournament_player_id ASC';
	}
	if ($isCards == 1) {
		$teamNumFilter = 'AND tournament_player_card > 0';
		$teamFilter = '';
		$orderFilter = 'tournament_player_card ASC';
	}

	$playersQuery=mysql_query("SELECT * FROM $tournamentPlayersTable WHERE tournament_player_tournament_id = $tourneyID $leagueFilter $teamFilter $teamNumFilter ORDER BY $orderFilter") 
		or die('ERROR getting player data from team '.mysql_error());
	while($player = mysql_fetch_array($playersQuery)) {
		if ($player['tournament_player_id'] == $playerID) {
			$playersDropDown.= "<option selected='selected' value=$player[tournament_player_id]>$player[tournament_player_firstname] $player[tournament_player_lastname]</option>\n";
		} else {
			$playersDropDown.= "<option value=$player[tournament_player_id]>$player[tournament_player_firstname] $player[tournament_player_lastname]</option>\n";
		}
	}
