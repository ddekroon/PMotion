...
function register($teamObj, $playerObj, $userID){
	...
	if($teamObj->teamPayMethod != 0) {
		...
		//mailWaivers();
		//header("Location: thankyoureg.htm"); //show the thank you for registering page, code will continue to run on this page until exit;
		exit;
	} else {
...
function updateTeam($teamObj, $playerObj, $userID){
	...
	header("Location: thankYouUpdate.htm");  //show the thank you for registering page, code will continue to run on this page until exit;
	$areComments = $teamObj->teamComments != '' ? 1 : 0;
