...
$teamsQuery = mysql_query("SELECT * FROM $teamsTable 
	...
	league_week_in_score_reporter AND $dayOfYear >= date_day_of_year_num AND team_dropped_out = 0 AND league_send_late_email = 1") 
...
while($teamArray = mysql_fetch_array($teamsQuery)) {
	...
	if ($lateEmailAllowed == 1) {
		//sendEmailsNotSMTP(array("alex@perpetualmotion.org"), 'scores@perpetualmotion.org', "Late Emails Allowed", "Late Emails Allowed");
		print($lateEmailAllowed);
	}
	else {
		//sendEmailsNotSMTP(array("alex@perpetualmotion.org"), 'scores@perpetualmotion.org', "Late Emails Not Allowed", "Late Emails Not Allowed");
		print($lateEmailAllowed);
	}
