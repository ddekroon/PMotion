...
<?php // VARIABLE DECLARATIONS
	$leagueQuery = "SELECT * FROM $leaguesTable 
	...
	while($leagueObj = $result->fetch_object()) {
		...
		$teamsQuery= "SELECT * FROM $teamsTable WHERE team_league_id = $curLeagueID AND team_num_in_league > 0
		...
		while($teamObj = $teamResult->fetch_object()) {
			...
			$spiritQuery= "SELECT * FROM $spiritScoresTable INNER JOIN $scoreSubmissionsTable ON 
		...
		if(count($team) > 0) {
			...
			<div class='league'>
				<table class='standings'>
					...
					printBottomData($league, $team); ?>
				</table>
                <br />
                <p><table align="center" style="width:75%;">
                	<?php for($i = 0 ; $i < 4; $i++) {
						printContactInfo($teams[$i], $i);
					} ?>
                </table></p>
			</div>
			<?php $team = array();
...
function printContactInfo($curTeam,$i) {
	
	global $leaguesTable, $playersTable; 
	$teamID=$curTeam->teamID;
	$captainQuery = mysql_query("SELECT * FROM $playersTable WHERE player_is_captain = 1 AND player_team_id = $teamID;");
