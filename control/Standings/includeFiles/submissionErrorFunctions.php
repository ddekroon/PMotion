...
function checkForErrors() {
	...
	global $matches, $games, $teamID, $leagueID;
	$error = '';
	$gameNum = 0; 
	$matchNum = 0;
    ...
    for ($i=0;$i<$matches;$i++) {
        ...
        if ($spiritScores[$i] && $spiritScores[$i] < 3.5 && strlen($matchComments[$i]) < 2) {
	...
	if($submitEmail != '' && !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
