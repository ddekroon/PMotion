<?php
	...
	for($i=1;$i<9;$i++) {
		$personQuery = mysql_query("SELECT * FROM $playersTable INNER JOIN $individualsTable ON $individualsTable.individual_player_id = $playersTable.player_id 
			...
			WHERE player_hear_method = $i  AND (player_is_captain = 1 OR player_is_individual = 1) AND season_year = $yearNum") 
	...
	# -------  Define Colors ----------------
	$bar_color=imagecolorallocate($img,102, 0, 102);
